...
class Model:
    ...
    def train(self):
        ...
        for epoch in range(self.args.epochs):
            ...
            running_loss = 0.0
            self.model.train()
            # data in self.loader，这样就是循环遍历每一个batch，一次拿到batch中的多张图片
            for data in self.loader:
                ...
                disps = self.model(left)   # 传入左图，预测左右视差图
                loss = self.loss_function(disps, [left, right])  # 传入左图和右图，计算损失
                loss.backward()
                self.optimizer.step()
