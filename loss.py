...
class MonodepthLoss(nn.modules.Module):
    """_summary_
        定义损失函数。这个比较有意思，它的loss_function也是用一个网络层的结构来实现的，
        也就是把图像传入这个类，它会自动调用forward， 然后后面再调用反向传播即可。
    """
    ...
    def apply_disparity(self, img, disp):
        ...
        # ! 这里可以看到，其实估计是视差图是一个比例，而不是原始的像素
        x_shifts = disp[:, 0, :, :]  # Disparity is passed in NCHW format with 1 channel
        flow_field = torch.stack((x_base + x_shifts, y_base), dim=3)
        # In grid_sample coordinates are assumed to be between -1 and 1
        output = F.grid_sample(img, 2*flow_field - 1, mode='bilinear',
                               padding_mode='zeros')  # padding_mode='zeros':对于越界的位置在网格中采用pixel value=0进行填充。
